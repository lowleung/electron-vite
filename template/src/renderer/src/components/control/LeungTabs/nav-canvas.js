export class NavCanvas {
  ...
  calcTabs() {
    ...
    const ws = []
    let totalWidth = 0
    this.offsetLeft = tabs.length > 0 ? tabs[0].offsetLeft : 0
    tabs.forEach((node) => {
      ws.push(totalWidth)
  ...
  draw(ctx, trueorfalse) {
    ...
    ctx.lineTo(-50, k + j)
    ctx.lineTo(this.offsetLeft - 50, k + j)
    if (opt.animating) {
      const m = this.getCurSpeed(opt.curDisX, opt.distance)
      l = Math.min(Math.abs(opt.distance), Math.abs(opt.curDisX + m)) * Math.sign(m)
    }
    ctx.lineTo(this.offsetLeft + f + tabWidthList[navindex] + l - g / 2, k + j)
    ctx.bezierCurveTo(
      this.offsetLeft + f + tabWidthList[navindex] + l - g / 2 + i,
      k + j,
      this.offsetLeft + f + tabWidthList[navindex] + l + g / 2 - i,
      tableHeight + j,
      this.offsetLeft + f + tabWidthList[navindex] + l + g / 2,
      tableHeight + j
    )
    if (opt.animating) {
      const o = tabWidthList[opt.nextIndex + 1] - tabWidthList[opt.nextIndex]
      ctx.lineTo(this.offsetLeft + f + tabWidthList[navindex] + o + l - g / 2, tableHeight + j)
      ctx.bezierCurveTo(
        this.offsetLeft + f + tabWidthList[navindex] + o + l - g / 2 + i,
        tableHeight + j,
        this.offsetLeft + f + tabWidthList[navindex] + o + l + g / 2 - i,
        k + j,
        this.offsetLeft + f + tabWidthList[navindex] + o + l + g / 2,
        k + j
      )
    } else {
      //lineTo 添加一个新点 宽度长度
      // ctx.lineTo(f + tabWidthList[navindex + 1] + l - g / 2, k + j)
      ctx.lineTo(this.offsetLeft + f + tabWidthList[navindex + 1] + l - g / 2, tableHeight + j)
      // this.calCurve(
      //   ctx,
      ...
      // )
      ctx.bezierCurveTo(
        this.offsetLeft + f + tabWidthList[navindex + 1] + l - g / 2 + i,
        tableHeight + j,
        this.offsetLeft + f + tabWidthList[navindex + 1] + l + g / 2 - i,
        k + j,
        this.offsetLeft + f + tabWidthList[navindex + 1] + l + g / 2,
        k + j
      )
    }
    ctx.lineTo(opt.width + 10, k + j)
